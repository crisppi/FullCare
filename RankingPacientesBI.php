...
$rangeStmt = $conn->query("
    SELECT
        MIN(data_intern_int) AS min_date,
        MAX(data_intern_int) AS max_date
    FROM tb_internacao
...
$sql = "
    SELECT
        COALESCE(p.nome_pac, 'Sem informações') AS paciente,
    ...
    FROM ({$baseSql}) base
    LEFT JOIN tb_paciente p ON p.id_paciente = base.fk_paciente_int
    GROUP BY paciente
...
<div class="bi-wrapper bi-theme">
    <div class="bi-header">
        <h1 class="bi-title">Ranking Pacientes</h1>
