<?php
$pageTitle = 'Desospitalizacao Precoce';
$pageSlug = 'bi/risco-desospitalizacao';
...
$summaryStmt = $conn->prepare("\n    SELECT\n        COUNT(*) AS casos,\n        SUM(CASE WHEN COALESCE(g.desospitalizacao_ges,'n') = 's' THEN 1 ELSE 0 END) AS desospitalizadas\n    FROM tb_internacao i\n    LEFT JOIN tb_gestao g ON g.fk_internacao_ges = i.id_internacao\n    {$internJoins}\n    WHERE {$internWhere}\n");
biBindParams($summaryStmt, $internParams);
$summaryStmt->execute();
$summary = $summaryStmt->fetch(PDO::FETCH_ASSOC) ?: [];

$casos = (int)($summary['casos'] ?? 0);
$desospitalizadas = (int)($summary['desospitalizadas'] ?? 0);
$desHospPct = $casos > 0 ? ($desospitalizadas / $casos) * 100 : 0.0;

$rowsStmt = $conn->prepare("\n    SELECT\n        h.nome_hosp AS hospital,\n        COUNT(*) AS casos,\n        SUM(CASE WHEN COALESCE(g.desospitalizacao_ges,'n') = 's' THEN 1 ELSE 0 END) AS desospitalizadas\n    FROM tb_internacao i\n    LEFT JOIN tb_hospital h ON h.id_hospital = i.fk_hospital_int\n    LEFT JOIN tb_gestao g ON g.fk_internacao_ges = i.id_internacao\n    {$internJoins}\n    WHERE {$internWhere}\n    GROUP BY h.id_hospital\n    HAVING h.id_hospital IS NOT NULL\n    ORDER BY desospitalizadas DESC\n    LIMIT 10\n");
...
<div class="bi-wrapper bi-theme">
    <div class="bi-header">
        <div>
            ...
            <div style="color: var(--bi-muted); font-size: 0.95rem;">Casos com sinalizacao de desospitalizacao.</div>
        </div>
    ...
    <div class="bi-panel">
        ...
        <div class="bi-kpis kpi-grid-4">
            <div class="bi-kpi kpi-compact">
                <small>Taxa de desospitalizacao</small>
                <strong><?= fmtPct($desHospPct, 1) ?></strong>
            </div>
            <div class="bi-kpi kpi-compact">
                <small>Casos sinalizados</small>
                <strong><?= fmtInt($desospitalizadas) ?></strong>
            </div>
            <div class="bi-kpi kpi-compact">
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($rows as $row): ?>
                        ...
                        $flag = (int)($row['desospitalizadas'] ?? 0);
                        $rate = $total > 0 ? ($flag / $total) * 100 : 0.0;
                        ?>
                        <tr>
