(function () {
    ...
    function openPanel() {
        const body = byId('parecer-visita-ia-body');
        const toggle = byId('btn-toggle-parecer-visita-ia');
    ...
    function setStatus(message, type) {
        const status = byId('parecer-visita-ia-status');
        if (!status) return;
        status.hidden = false;
    ...
    function renderResult(title, text, extraHtml) {
        const content = byId('parecer-visita-ia-content');
        if (!content) return;
    ...
    function buildUtiOpinion() {
        const rel = cleanText((byId('rel_visita_vis') || {}).value);
        const acoes = cleanText((byId('acoes_int_vis') || {}).value);
        const prog = cleanText((byId('programacao_enf') || {}).value);
        const text = [rel, acoes, prog].join(' ').toLowerCase();
        const sinais = [];
        [
        ...
        return {
            pertinente: sinais.length > 0,
            sinais,
            texto: [
                sinais.length ? 'Parecer preliminar: há indícios clínicos compatíveis com suporte intensivo.' : 'Parecer preliminar: não encontrei critérios fortes de UTI no texto disponível.',
        ...
        if (readBtn && input) {
            ...
            input.addEventListener('change', async function () {
                ...
                try {
                    ...
                    setValue('rel_visita_vis', text, true);
